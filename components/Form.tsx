...
export function Form({ _id }) {
  ...
  return (
    <form onSubmit={handleSubmit((data) => {
      ...
      <p>Name:</p>
      <input {...register('name', { required: true })} className='border-2 p-1 border-gray-300 rounded-md outline-none focus:border-[#2570d1] w-72' />
      {errors.name && <p className='text-red-700'>Name is required.</p>}
      <p className='mt-[0.8rem]'>Comment:</p>
      <textarea {...register('text', {required: true})} rows={5} className='border-2 p-1 border-gray-300 rounded-md resize-none outline-none focus:border-[#2570d1]' />
